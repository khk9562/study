...
| AI 챗봇         | **@anthropic-ai/sdk** (Claude)                   | 서버 라우트에서만 호출, 스트리밍                                           |
| 마크다운 렌더   | **react-markdown + remark-gfm**                  | 챗봇 답변 렌더 전용                                                        |

`package.json` 핵심 의존성:

```
@anthropic-ai/sdk · framer-motion · lucide-react · next · next-themes · react · react-dom · react-markdown · remark-gfm
devDeps: tailwindcss@4 · @tailwindcss/postcss
```

> 챗봇 사용 시 환경변수 `ANTHROPIC_API_KEY` 필요(로컬 `.env.local` / Vercel env). 모델은 `CHAT_MODEL`(기본 `claude-haiku-4-5`).
...
@theme inline {
  ...
  --font-sans: 'Pretendard Variable', Pretendard, var(--font-geist-sans),
...
높이 `h-16`, 내부 `max-w-5xl px-5`. 우측에 PDF 메뉴 + 테마 토글 + **AI 챗봇 버튼**.

### 6.8 AI 챗봇 위젯 (`components/ui/ChatWidget.tsx` + `app/api/chat/route.ts`)

포트폴리오 데이터(`lib/profile.ts`, `lib/projects.ts`)에 한정해 답하는 우측 Drawer 챗봇. 헤더 봇 버튼으로 토글.

**구조**

- **서버 라우트** `app/api/chat/route.ts` (`runtime='nodejs'`, `dynamic='force-dynamic'`): `@anthropic-ai/sdk`로 Claude 호출. profile+projects를 **시스템 프롬프트에 주입**하고 "이 포트폴리오 내용만 답하고 무관한 질문은 정중히 거절" 가드. **스트리밍**(`text/plain`) 반환. API 키(`ANTHROPIC_API_KEY`)는 서버에서만 사용 → 클라이언트 비노출. 시스템 프롬프트엔 `cache_control:{type:'ephemeral'}`로 프롬프트 캐싱. 모델은 `CHAT_MODEL` 환경변수(기본 `claude-haiku-4-5`). 남용 방지: 히스토리 20개·메시지 2000자 상한.
- **클라이언트** `ChatWidget.tsx`: 봇 버튼 + framer-motion Drawer. 닫기는 **X 버튼으로만**(바깥 클릭·ESC 없음).

**핵심 UI 규칙**

- **Drawer는 `createPortal`로 `document.body`에 렌더**. 헤더의 `backdrop-filter`(스크롤 시 `backdrop-blur`)가 자식 `fixed`의 기준(containing block)을 헤더로 바꿔 높이가 짜부되는 버그를 회피. 높이는 `fixed inset-y-0 right-0`(= top0+bottom0)로 뷰포트 전체 고정, `h-full`(%) 사용 금지.
- 등장: `initial={{x:'100%'}} animate={{x:0}}` spring(`damping:30, stiffness:300`). `w-full max-w-md`, `z-[70]`, `border-l border-border bg-background`.
- 메시지 영역: `chat-scroll overscroll-contain overflow-y-auto`로 **스크롤 체이닝 차단**(끝에서 페이지로 전파 안 됨) + §9.3 커스텀 스크롤바.
- 말풍선: 사용자 `bg-accent text-accent-foreground`(우측), AI `bg-surface-2 text-foreground`(좌측). AI 답변은 **`react-markdown`+`remark-gfm`으로 마크다운 렌더**(요소별 Tailwind override), 사용자 메시지는 평문.
- 답변 하단 우측 액션: 정상 답변 **복사 버튼**(클릭 시 1.5s 체크 피드백), 에러 답변 **재시도 버튼**(`↺`). 재시도는 실패한 `⚠️` 메시지를 히스토리에서 제거 후(`withoutTrailingError`) 직전 user 메시지로 재요청. 전송/재시도는 공용 `runCompletion(history)` 경로.
...
- `dark:block`(다크에서만), `no-print`, 마스크 그라데이션(좌→우 페이드 + 하단 페이드)으로 한쪽만 진하게.
- **뷰포트 좌상단 고정**: `fixed left-0 top-0 h-screen -z-10`. 스크롤로 다른 섹션에 가도 항상 같은 자리에 보인다. (섹션이 stacking context를 만들지 않아 `-z-10` 레이어 유지 → 콘텐츠 뒤·body 배경 위에 깔림. `fixed`라 Hero `overflow-hidden`에 잘리지 않고, 별 클리핑은 컴포넌트 자체 `overflow-hidden`이 담당.)
...
### 9.3 스크롤바 (전체 숨김 + 챗봇 전용 커스텀)

- **전체 페이지 스크롤바 숨김**(스크롤 기능은 유지): `html`에만 적용해 챗봇 등 다른 스크롤 영역엔 영향 없음.
- **챗봇 메시지 영역 전용** `.chat-scroll`: 너비 `6px`로 얇게, 색은 포인트색(`--accent`)에 **투명도 22%**(hover 40%) — `color-mix`로 라이트/다크 자동 대응.
...
> 스크롤 컨테이너에는 `overscroll-contain`을 함께 줘서 끝에 닿아도 페이지로 스크롤이 전파되지 않게 한다.
...
| `.print-page-break` | (no-op) 과거 강제 페이지 나눔용. **현재 비활성** — 강제 넘김으로 페이지마다 여백이 생겨서 제거. 내용이 이어서 페이지를 채운다. |
| `.print-avoid-break` | 카드·이미지 등 한 덩어리가 페이지 경계에서 반토막 나는 것 방지(`break-inside: avoid`) |
...
app/            라우트(App Router), globals.css, layout.tsx
  api/chat/     AI 챗봇 서버 라우트 (Claude 스트리밍, 서버 전용)
  print/[type]/ 인쇄 전용 라우트 (resume·career·portfolio)
  projects/[slug]/ 프로젝트 상세 (generateStaticParams)
components/
  ui/         프리미티브 (Badge, Reveal, SectionHeading, ThemeToggle, Starfield, ChatWidget, icons …)
